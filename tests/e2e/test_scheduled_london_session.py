...
from services.schedule_presets import london_preset
...
def _chat(payload: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "usage": {"prompt_tokens": 30, "completion_tokens": 20, "total_tokens": 50, "cost": 0.0001},
    }


@pytest.mark.asyncio
async def test_scheduled_london_session_runs_with_mock_tv_unified(tmp_path: Path) -> None:
    def handler(req: httpx.Request) -> httpx.Response:
        ...
        if kind == "synthesis":
            return httpx.Response(200, json=_chat("Analisis Londres OK"))
    ...
    deps = CognitiveDeps(
        ...
        cost_monitor=CostMonitor(db_path=tmp_path / "cost_e2e_sched.db"),
        mcp=_FakeMcp(),
        prompt_loader=default_loader(),
    )
    agent = AetheerCognitiveAgent(deps)
    scheduler = AetheerScheduler(
        cognitive_agent=agent,
        config=[london_preset(time(7, 0))],
        flags=_SchedulerFlags(enabled=True),
    )
    try:
        response = await scheduler._run_preset("london")
    finally:
        await client.aclose()

    assert response is not None
    assert response.approved is True
    assert response.trace_id.startswith("sched-london-")
