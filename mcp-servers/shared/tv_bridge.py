...
class TVBridge:
    ...
    async def reconnect_to_active_target(self) -> bool:
        """Reconectar WebSocket al target activo (necesario después de tab switch)."""
        if self._listener_task and not self._listener_task.done():
            self._listener_task.cancel()
            try:
                await self._listener_task
    ...
    async def get_all_targets(self) -> list:
        """Lista todos los targets CDP disponibles en el puerto."""
        try:
            if self._session is None or self._session.closed:
                self._session = aiohttp.ClientSession()
            async with self._session.get(
    ...
    async def activate_target(self, target_id: str) -> bool:
        """Activar un target CDP por ID (para tab switch, equivalente a json/activate)."""
        try:
            if self._session is None or self._session.closed:
                self._session = aiohttp.ClientSession()
            async with self._session.get(
            ...
            logger.error(f"activate_target {target_id} falló: {e}")
            return False
